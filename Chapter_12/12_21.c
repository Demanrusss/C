...
void * searchList(SYMBOLNODEPTR, char);
...
int main() {
    ...
    scanf("\n%c", &value);
    printf("The position of your char has been found in %p\n", searchList(node1Ptr, value));
...
void * searchList(SYMBOLNODEPTR sPtr, char letterToFind) {

    if (sPtr->symbol == letterToFind)
        return sPtr;
    else
        searchList(sPtr->nextPtr, letterToFind);
}
